...
def plot_test_auc(results_dir, output_dir):
    """Create a bar plot of test AUC by model and encoder."""
    ...
    # Filter out rows with NaN AUC values
    valid_auc_df = plot_df.dropna(subset=['AUC'])

    if valid_auc_df.empty:
        print("No valid AUC values found for plotting")
        return

    # Plot test AUC with error bars
    x_positions = np.arange(len(valid_auc_df))
    bar_colors = [colors[model_encoder] for model_encoder in valid_auc_df['model_encoder']]

    bars = plt.bar(x_positions, valid_auc_df['AUC'], yerr=valid_auc_df['AUC_SE'],
    ...
    for bar, auc in zip(bars, valid_auc_df['AUC']):
        height = bar.get_height()
        plt.text(bar.get_x() + bar.get_width() / 2., height + 0.02,
                 f'{auc:.3f}', ha='center', va='bottom', fontsize=9, fontweight='bold')

    # Format the plot
    plt.title('Test AUC by Model and Encoder', fontsize=14, fontweight='bold')
    plt.ylabel('Test AUC', fontsize=12)
    plt.xticks(x_positions, valid_auc_df['Model'], fontsize=9)
    plt.ylim(0, min(1.0, valid_auc_df['AUC'].max() + valid_auc_df['AUC_SE'].max() + 0.05))
    plt.grid(axis='y', linestyle='--', alpha=0.3)

    # Add note about standard error
    plt.text(0.02, 0.02, f'Error bars show standard error (n={valid_auc_df["n_runs"].iloc[0]} runs)',
    ...
    # Save the figure
    plt.savefig(output_dir / 'test_auc_comparison.pdf', dpi=600, bbox_inches='tight')
    plt.savefig(output_dir / 'test_auc_comparison.png', dpi=300, bbox_inches='tight')
    plt.close()

    print("Test AUC comparison plot generated successfully")


def main():
    """Parse arguments and generate the plot."""
    parser = argparse.ArgumentParser(description="Generate test AUC bar plot")
    ...
    # Generate the plot
    plot_test_auc(results_dir, output_dir)
